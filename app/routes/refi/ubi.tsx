...
export function meta() {
  return [{ title: "UBI | CARBON Copy ReFi Impact Dashboard" }];
}
...
export function loader() {
  ...
  const ubi = fetch("http://localhost:8000/api/aggregate-metrics/2").then(
    (res) => res.json()
  );

  return { ubi, chartsData };
}

export default function UBI() {
  const { ubi, chartsData } = useLoaderData<typeof loader>();
