...
export function meta() {
  return [{ title: "Lending | CARBON Copy ReFi Impact Dashboard" }];
}
...
export function loader() {
  ...
  const lending = fetch("http://localhost:8000/api/aggregate-metrics/2").then(
    (res) => res.json()
  );

  return { lending, chartsData };
}

export default function Lending() {
  const { lending, chartsData } = useLoaderData<typeof loader>();
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 overflow-x-hidden relative">
      <div>
        <h2 className="md:text-[27px] text-[17px] font-[500]">Lending</h2>
